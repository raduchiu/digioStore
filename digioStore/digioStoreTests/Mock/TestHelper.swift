...
class LoadDataHelper {
    ...
    func parseMockResponseData(_ data: Data) -> StoreResponse? {
        do {
            let decoder = JSONDecoder()
            let response = try decoder.decode(StoreResponse.self, from: data)
            return response
        } catch {
            print("Failed to parse products.json data: \(error)")
            return nil
        }
    }
}
